...
export class ExpensesController {
  ...
  @Post()
  create(@Param('groupId') groupId: string, @CurrentUser('id') userId: string, @Body() dto: CreateExpenseDto) {
    return this.expensesService.create(groupId, userId, dto);
  }

  @UseGuards(JwtAuthGuard, GroupMemberGuard)
  @Post('bulk')
  bulkCreate(@Param('groupId') groupId: string, @CurrentUser('id') userId: string, @Body() dto: CreateExpenseBulkDto) {
    return this.expensesService.bulkCreate(groupId, userId, dto.expenses);
  }

  @UseGuards(JwtAuthGuard, GroupMemberGuard)
  @Get()
  findAll(@Param('groupId') groupId: string, @CurrentUser('id') userId: string, @Query() query: FindAllExpensesDto) {
  ...
  @UseGuards(JwtAuthGuard, GroupMemberGuard, GroupOwnerGuard)
  @Delete('bulk')
  bulkDelete(@Param('groupId') groupId: string, @CurrentUser('id') userId: string, @Body() dto: DeleteExpensesBulkDto) {
    return this.expensesService.bulkDelete(groupId, userId, dto.ids);
  }

  @UseGuards(JwtAuthGuard, GroupMemberGuard, GroupOwnerGuard)
  @Delete(':id')
  delete(@Param('groupId') groupId: string, @Param('id') id: string, @CurrentUser('id') userId: string) {
